...
The spec version this repository targets is **omnist-spec v0.21.0-beta** (pinned via the `vendor/omnist-spec` git submodule). Pass/fail/skip counts will be shipped alongside every release per spec §10.3.
...
**Releasing is automated via `.github/workflows/release.yml`**, triggered by pushing a `v*` tag (e.g. `v0.2.5-alpha`). It runs as two jobs:
